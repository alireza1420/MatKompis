...
Extract slots:
- meal_recommendation: query, diet, quantity, nutrient, level (e.g. high, low), meal_type (optional: breakfast, lunch, dinner)
- recipe_query: ingredient, quantity
...
Examples:
{{"intent": "meal_recommendation", "slots": {{"query": "Find a high-protein vegan meal","diet": "vegan", "nutrient": "protein", "level": "high"}}}}
{{"intent": "meal_recommendation", "slots": {{"diet": "pescatarian","quantity": 2,"ingredient": "salmon"}}}}
...
def answer_query(intent: str, slots: Dict[str, Any]) -> str:
    if intent == "product_nutrient":
        ...
        if nutrient and nutrient != 'nutrition':
            value = None
            for k, v in product["nutrition"].items():
                if nutrient.lower() in k.lower():
                    value = v
        ...
        # If no specific nutrient, list all
        nutrition_lines = [f"{k}: {v}" for k, v in product["nutrition"].items()]
        nutrition_text = "\n".join(nutrition_lines)
        return f"Nutrition info for {product['name']} per 100 g:\n{nutrition_text}\nSource: {product['url']}"
    ...
    #         + "\n\nAsk: 'show <title>' or 'show <id>' to see full details.")
        ...
        query_text = slots.get("query")
        retrieved = retrieve_recipes(query_text, top_k=5)
        ...
        Based on the following recipe database, recommend the best options for this request:
        ...
        Please answer in a friendly and concise way.
        """
        model = genai.GenerativeModel("models/gemini-2.5-flash")
        answer = model.generate_content(rag_prompt).text
        return answer

    # elif intent == "meal_recommendation":
        query = slots.get("query", "")
        diet = slots.get("diet")
        quantity = slots.get("quantity", 5)
        nutrient = slots.get("nutrient")
        level = slots.get("level")
        meal_type = slots.get("meal_type")

        recipes = retrieve_recipes(query, top_k=10)
        ...
        if not recipes:
            return f"Sorry, I couldn’t find any recipes."
        
        import random
        selected_recipes = random.sample(recipes, min(quantity, len(recipes)))
        lines = [f"{i+1}. 🍽️ {r['title']}" for i, r in enumerate(selected_recipes)]
        return f"🌱 Here are some recipes" + (f" rich in {nutrient}" if nutrient else "") + ":\n" + "\n".join(lines)
        ...
        # hits = list_recipes_by_ingredient(ingredient, limit=quantity)
        hits = retrieve_recipes(ingredient, top_k=quantity)
        if not hits:
            return f"Sorry, I couldn’t find recipes with {ingredient}."

        context_text = "\n".join([f"- {r['title']}: {r.get('instructions', '')}, {r.get('ingredients', [])}" for r in hits])
        
        rag_prompt = f"""
        You are a helpful cooking assistant.

        A user asked for recipes with the ingredient: "{ingredient}"

        Here are some relevant recipes:
        {context_text}

        Please recommend the top {quantity} recipes in a friendly and concise way.
        Format:
